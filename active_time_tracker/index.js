...
// tabId -> timerId（URL 变化的延迟处理，确保能拿到 webNavigation 的 transition 信息）
const pendingTabUrlUpdatesByTabId = new Map();

// tabId -> markedAt（用于识别“前进/后退”导航，避免把书签归因错误带到回退页面）
const recentForwardBackNavigationByTabId = new Map();
const FORWARD_BACK_MARK_TTL_MS = 2000;

// tabId -> { url, title }（非书签页面负缓存：避免窗口焦点/激活时反复匹配与刷屏）
const recentNonBookmarkPageByTabId = new Map();

function isTrackableHttpUrl(url) {
    return typeof url === 'string' && (url.startsWith('http://') || url.startsWith('https://'));
}

function rememberNonBookmarkPage(tabId, url, title) {
    if (typeof tabId !== 'number') return;
    if (typeof url !== 'string' || url.length === 0) return;
    recentNonBookmarkPageByTabId.set(tabId, { url, title: typeof title === 'string' ? title : '' });
}

function isKnownNonBookmarkPage(tabId, url, title) {
    const cached = recentNonBookmarkPageByTabId.get(tabId);
    if (!cached) return false;
    if (cached.url !== url) return false;
    const normalizedTitle = typeof title === 'string' ? title : '';
    return cached.title === normalizedTitle;
}

function clearNonBookmarkPage(tabId) {
    recentNonBookmarkPageByTabId.delete(tabId);
}

function markForwardBackNavigation(tabId) {
    if (typeof tabId !== 'number') return;
    recentForwardBackNavigationByTabId.set(tabId, Date.now());
}

function consumeForwardBackNavigation(tabId) {
    const markedAt = recentForwardBackNavigationByTabId.get(tabId);
    if (!markedAt) return false;
    recentForwardBackNavigationByTabId.delete(tabId);
    return (Date.now() - markedAt) <= FORWARD_BACK_MARK_TTL_MS;
}

function scheduleTabUrlUpdate(tabId, newUrl, tab) {
    const existingTimer = pendingTabUrlUpdatesByTabId.get(tabId);
    if (existingTimer) {
        clearTimeout(existingTimer);
        pendingTabUrlUpdatesByTabId.delete(tabId);
    }

    const timerId = setTimeout(() => {
        pendingTabUrlUpdatesByTabId.delete(tabId);
        handleTabUrlChange(tabId, newUrl, tab).catch((error) => {
            console.warn('[ActiveTimeTracker] 处理URL变化失败:', error);
        });
    }, 0);

    pendingTabUrlUpdatesByTabId.set(tabId, timerId);
}
...
function shouldSkipStartForKnownNonBookmark(tabId, url, title) {
    if (!isKnownNonBookmarkPage(tabId, url, title)) return false;
    return !getValidAutoBookmarkAttribution(tabId);
}
...
async function rebuildBookmarkCache() {
    ...
    recentNonBookmarkPageByTabId.clear();
...
async function handleTabActivated(activeInfo) {
    ...
    try {
        ...
        // 不可追踪协议：直接跳过（但保留“书签打开归因”的兜底能力）
        if (!isTrackableHttpUrl(tabUrl) && !getValidAutoBookmarkAttribution(tabId)) {
            rememberNonBookmarkPage(tabId, tabUrl, tabTitle);
            return;
        }

        // 同一 tab 同一页面已判定为“非书签”，且没有有效归因：跳过反复匹配
        if (!session && shouldSkipStartForKnownNonBookmark(tabId, tabUrl, tabTitle)) {
            return;
        }

        if (!session) {
            session = new SessionData(tabId, windowId, tabUrl, tabTitle);
...
async function handleTabUpdated(tabId, changeInfo, tab) {
    ...
    if (changeInfo.url) {
        clearNonBookmarkPage(tabId);
        scheduleTabUrlUpdate(tabId, changeInfo.url, tab);
    }
...
async function handleTabUrlChange(tabId, newUrl, tab) {
    if (!trackingEnabled) return;
    if (!newUrl) return;

    clearNonBookmarkPage(tabId);

    // 前进/后退：属于用户主动导航，不应继续沿用“书签打开归因”
    const isForwardBack = consumeForwardBackNavigation(tabId);
    if (isForwardBack) {
        autoBookmarkAttributionByTabId.delete(tabId);
    }

    const resolvedTab = (() => {
        if (tab && typeof tab === 'object' && typeof tab.windowId === 'number') return tab;
        return null;
    })();

    let tabInfo = resolvedTab;
    if (!tabInfo) {
        try {
    ...
    // 不可追踪协议：结束旧会话但不创建新会话，避免反复匹配
    if (!isTrackableHttpUrl(newUrl) && !getValidAutoBookmarkAttribution(tabId)) {
        if (session) {
            const ended = session.end();
            if (ended && ended.accumulatedActiveMs >= CONFIG.MIN_ACTIVE_MS) {
                await saveSession(ended);
        ...
        autoBookmarkAttributionByTabId.delete(tabId);
        rememberNonBookmarkPage(tabId, newUrl, tabInfo?.title || '');
        return;
    }
    ...
    if (!isForwardBack && session && !session.forceBookmarkTracking) {
        const attribution = getValidAutoBookmarkAttribution(tabId);
        if (attribution && (Date.now() - (attribution.updatedAt || 0)) <= CONFIG.AUTO_BOOKMARK_REDIRECT_WINDOW_MS) {
            const match = isBookmarkUrl(newUrl, tabInfo?.title || '');
            if (!match.isBookmark) {
                session.applyBookmarkAttribution(attribution);
                session.url = newUrl;
                await reportAttributedBookmarkOpenOnce(session, attribution);
    ...
    if (session && session.forceBookmarkTracking) {
        ...
        if (withinRedirectWindow && !isForwardBack) {
            session.url = newUrl;
            return;
        }

        // 超出归因窗口（或用户前进/后退）：结束旧会话，并清理归因标记，让后续按普通规则运行
        const ended = session.end();
        if (ended && ended.accumulatedActiveMs >= CONFIG.MIN_ACTIVE_MS) {
            await saveSession(ended);
        }
    ...
    try {
        ...
        if (activeTab[0] && activeTab[0].id === tabId) {
            const newSession = new SessionData(tabId, tabInfo.windowId, newUrl, tabInfo.title);
...
async function handleTabRemoved(tabId, removeInfo) {
    const pendingTimer = pendingTabUrlUpdatesByTabId.get(tabId);
    if (pendingTimer) {
...
function handleWebNavigationCommitted(details) {
    ...
    if (qualifiers.includes('forward_back')) {
        markForwardBackNavigation(tabId);
        // 这里也提前清掉归因，降低 race 风险（URL 处理里也会兜底再清一次）
        autoBookmarkAttributionByTabId.delete(tabId);
    }
}

function handleWebNavigationHistoryStateUpdated(details) {
    if (!details || details.frameId !== 0) return;
    const tabId = details.tabId;
    if (typeof tabId !== 'number') return;
...
async function resumeActiveTabInWindow(windowId) {
    try {
        ...
        if (tabs[0]) {
            ...
            if (tabUrl) {
                if (!isTrackableHttpUrl(tabUrl) && !getValidAutoBookmarkAttribution(tabs[0].id)) {
                    rememberNonBookmarkPage(tabs[0].id, tabUrl, tabTitle);
                    return;
                }

                if (!session && shouldSkipStartForKnownNonBookmark(tabs[0].id, tabUrl, tabTitle)) {
                    return;
                }
            }
            ...
            // 如果没有现有会话，创建一个新的
            if (!session && tabUrl) {
                session = new SessionData(tabs[0].id, windowId, tabUrl, tabTitle);
                ...
                if (focusedWindow && focusedWindow.focused) {
                    ...
                    if (restoredCount === 0) {
                        ...
                        if (tabs[0] && tabs[0].url && !activeSessions.has(tabs[0].id)) {
                            if (!isTrackableHttpUrl(tabs[0].url) && !getValidAutoBookmarkAttribution(tabs[0].id)) {
                                rememberNonBookmarkPage(tabs[0].id, tabs[0].url, tabs[0].title || '');
                            } else {
...
function setupEventListeners() {
    ...
    // 前进/后退识别（避免“书签打开归因”在回退后仍继续生效）
    if (browserAPI.webNavigation) {
        if (browserAPI.webNavigation.onCommitted) {
            browserAPI.webNavigation.onCommitted.addListener(handleWebNavigationCommitted);
        }
        if (browserAPI.webNavigation.onHistoryStateUpdated) {
            browserAPI.webNavigation.onHistoryStateUpdated.addListener(handleWebNavigationHistoryStateUpdated);
        }
    }
