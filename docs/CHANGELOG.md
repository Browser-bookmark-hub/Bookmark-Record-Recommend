...
- 导出生态增强：
  - 推送与分析、手动导出均可选择 GitHub、本地或两者。
  - GitHub 手动导出按 `manual-export/<类别>/<日期>/` 规范存放，不参与常规同步或拉取。
  - 关联记录支持右键任意条目导出上下文、设置前后范围并保留橙色选中语义；点击排行新增单项导出。
- 体验与文档整理：
  - 推荐模式新增备份恢复与 S 值重算说明；favicon 预热限制为 150 个去重域名。
  - 补充书签事件 API 审计与多语言文档准备。
