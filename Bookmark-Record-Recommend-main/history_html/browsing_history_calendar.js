...
function t(key, ...args) {
    ...
    if (!dict || typeof currentLang === 'undefined') {
        return key; // 降级方案
    }
    let text = dict[key] ? dict[key][currentLang] : key;
    // 替换占位符 {0}, {1}, etc.
...
function tw(index) {
    // 获取星期几的翻译（缩写）
    const dict = getI18nDict();
    if (!dict || typeof currentLang === 'undefined' || !dict.calendarWeekdays || !dict.calendarWeekdays[currentLang]) {
        return ['周日', '周一', '周二', '周三', '周四', '周五', '周六'][index];
    }
    return dict.calendarWeekdays[currentLang][index] || '';
}

function twFull(index) {
    // 获取星期几的完整名称
    const dict = getI18nDict();
    if (!dict || typeof currentLang === 'undefined' || !dict.calendarWeekdaysFull || !dict.calendarWeekdaysFull[currentLang]) {
        return ['周日', '周一', '周二', '周三', '周四', '周五', '周六'][index];
    }
    return dict.calendarWeekdaysFull[currentLang][index] || '';
}

function tm(index) {
    // 获取月份名称的翻译 (0-11)
    const dict = getI18nDict();
    if (!dict || typeof currentLang === 'undefined' || !dict.calendarMonthNames || !dict.calendarMonthNames[currentLang]) {
        return ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'][index];
    }
    return dict.calendarMonthNames[currentLang][index] || '';
}
...
const BROWSING_CLICK_DERIVE_YIELD_EVERY = 600;
const BROWSING_BOOKMARK_REFS_CACHE_TTL_MS = 60 * 1000;

function yieldBrowsingClickDerivation() {
    return new Promise(resolve => setTimeout(resolve, 0));
}

function shouldYieldBrowsingClickDerivation(processedCount, yieldEvery = BROWSING_CLICK_DERIVE_YIELD_EVERY) {
...
function normalizeBrowsingHistoryMetaEntries(entries = []) {
    const meta = {};
    ...
    const schemaVersion = Number(meta[BROWSING_HISTORY_META_SCHEMA_VERSION] || meta.schemaVersion || 0) || 0;
    return { ...meta, schemaVersion, lastSyncTime, lastFullSyncTime: lastSyncTime || Number(meta.lastFullSyncTime || 0) || 0 };
}
...
function openBrowsingHistoryCacheDB() {
    ...
    browsingHistoryCacheDbPromise = new Promise((resolve) => {
        ...
        request.onerror = () => {
            console.warn('[BrowsingHistoryCalendar] 打开缓存DB失败:', request.error);
            resolve(null);
        };
    ...
    return new Promise((resolve) => {
        ...
        const finish = () => {
            ...
            if (!records || records.length === 0) {
                resolve(meta.lastSyncTime ? { ...meta, schemaVersion: BROWSING_HISTORY_LIBRARY_SCHEMA_VERSION, dbName: BROWSING_HISTORY_CACHE_DB_NAME, records: [] } : null);
                return;
            }
            const normalizedRecords = records.map(entry => [entry.dateKey, entry.items || []]);
            resolve({ ...meta, schemaVersion: BROWSING_HISTORY_LIBRARY_SCHEMA_VERSION, dbName: BROWSING_HISTORY_CACHE_DB_NAME, records: normalizedRecords });
        };
        ...
        metaRequest.onsuccess = () => {
            meta = normalizeBrowsingHistoryMetaEntries(metaRequest.result || []);
            finish();
        };
        metaRequest.onerror = () => {
            meta = {};
...
async function readBrowsingHistoryCacheFromIDBRange(startKey, endKey) {
    const db = await openBrowsingHistoryCacheDB();
    if (!db) return null;
    ...
    return new Promise((resolve) => {
        ...
        const recordsStore = tx.objectStore(BROWSING_HISTORY_CACHE_DB_STORE);
        const metaStore = tx.objectStore(BROWSING_HISTORY_CACHE_DB_META);
        const range = (startKey && endKey) ? IDBKeyRange.bound(startKey, endKey) : null;
        const records = [];
        const cursorRequest = range ? recordsStore.openCursor(range) : recordsStore.openCursor();
        const metaRequest = metaStore.getAll();
        let meta = {};
...
async function readHistoryCacheValue(key) {
    if (key !== BROWSING_HISTORY_CACHE_KEY) return null;
    return await readBrowsingHistoryCacheFromIDB();
}

async function readHistoryCacheRange(startKey, endKey) {
    return await readBrowsingHistoryCacheFromIDBRange(startKey, endKey);
}
...
class BrowsingHistoryCalendar {
    constructor() {
        ...
        this.loadedRangeKeys = null;
        this.viewDataLoading = false;
        this.loadViewSeq = 0;
        this.databaseSyncSeq = 0;
        this.bookmarkClickRefsCache = null;
        this.bookmarkClickRefsPromise = null;
        ...
        this.initPromise = this.init()
            .catch((error) => {
                console.error('[BrowsingHistoryCalendar] 初始化失败:', error);
            })
            .finally(() => {
    ...
    async restoreBrowsingHistoryCache() {
        try {
            const range = this.getViewDateRange();
            ...
            console.warn('[BrowsingHistoryCalendar] 恢复缓存失败:', error);
            return false;
        }
    }

    async saveBrowsingHistoryCache() {
        console.warn('[BrowsingHistoryCalendar] 全局浏览历史缓存由 background 维护，前端不再直接写入。');
        return false;
    }

    async buildBookmarkClickReferenceSets(options = {}) {
        const force = options.force === true;
        const now = Date.now();
        if (
            !force &&
            this.bookmarkClickRefsCache &&
            (now - Number(this.bookmarkClickRefsCache.loadedAt || 0)) < BROWSING_BOOKMARK_REFS_CACHE_TTL_MS
        ) {
            return this.bookmarkClickRefsCache.refs;
        }

        if (!force && this.bookmarkClickRefsPromise) {
            return this.bookmarkClickRefsPromise;
        }
        ...
        this.bookmarkClickRefsPromise = (async () => {
            try {
                const tree = await browserAPI.bookmarks.getTree();
                if (tree && tree[0]) {
                    this.bookmarkFolderPaths = new Map();
                    this.bookmarkNodesByUrl = new Map();
                    this.collectBookmarkUrlsAndTitles(tree[0], refs.urls, refs.titles, [], refs.titleDomainMap);
                }
            } catch (error) {
                console.warn('[BrowsingHistoryCalendar] 获取书签集合失败:', error);
            }
            this.bookmarkClickRefsCache = { refs, loadedAt: Date.now() };
            return refs;
        })().finally(() => {
            this.bookmarkClickRefsPromise = null;
        });
    ...
    isBookmarkClickRecord(record, refs) {
        if (!record || !refs) return false;
        const url = String(record.url || '').trim();
        if (!url) return false;
        if (refs.urls?.has(url)) return true;

        const title = typeof record.title === 'string' ? record.title.trim() : '';
        if (!title || !refs.titles?.has(title)) return false;

        const allowedDomains = refs.titleDomainMap?.get(title);
        if (!allowedDomains || allowedDomains.size === 0) return false;
        const recordDomain = getUrlDomain(url);
        return !!recordDomain && allowedDomains.has(recordDomain);
    }

    normalizeBookmarkClickRecord(record, options = {}) {
        const visitTime = typeof record.visitTime === 'number'
            ? record.visitTime
            : (record.dateAdded || Date.now());
        const dateAdded = new Date(record.dateAdded || visitTime);
        const folderPath = this.bookmarkFolderPaths?.get(record.url) || record.folderPath || [];
        const visitKeySet = options.visitKeySet || this.visitKeySet;
        visitKeySet.add(`${record.url}|${visitTime}`);
        return {
            id: record.id,
    ...
    async loadViewRangeFromCache(startKey, endKey) {
        const loadSeq = ++this.loadViewSeq;
        const cached = await readHistoryCacheRange(startKey, endKey);
        const refs = await this.buildBookmarkClickReferenceSets();
        const nextBookmarksByDate = new Map();
        const nextVisitKeySet = new Set();
        let processed = 0;

        if (cached && Array.isArray(cached.records)) {
            for (const [dateKey, records] of cached.records) {
                if (loadSeq !== this.loadViewSeq) return false;
                if (!Array.isArray(records) || !records.length) continue;
                const normalized = [];
                for (const record of records) {
                    processed += 1;
                    if (this.isBookmarkClickRecord(record, refs)) {
                        normalized.push(this.normalizeBookmarkClickRecord(record, { visitKeySet: nextVisitKeySet }));
                    }
                    if (shouldYieldBrowsingClickDerivation(processed)) {
                        await yieldBrowsingClickDerivation();
                    }
                    if (loadSeq !== this.loadViewSeq) return false;
                }
                if (normalized.length) {
                    nextBookmarksByDate.set(dateKey, normalized);
                }
            }
        }
        ...
        nextBookmarksByDate.forEach((records, dateKey) => {
            this.bookmarksByDate.set(dateKey, records);
        });
        this.visitKeySet = nextVisitKeySet;
        this.historyCacheMeta = {
            lastSyncTime: cached?.lastSyncTime || this.historyCacheMeta.lastSyncTime || Date.now()
        };
        this.historyCacheRestored = !!cached;
        this.loadedRangeKeys = { startKey, endKey };
        return !!cached;
    }

    getViewDateRange() {
        const now = new Date();
        const year = Number.isFinite(this.currentYear) ? this.currentYear : now.getFullYear();
        const month = Number.isFinite(this.currentMonth) ? this.currentMonth : now.getMonth();
        const currentDay = this.currentDay ? new Date(this.currentDay) : now;

        if (!this.currentWeekStart) {
            const today = new Date(now);
            const todayDay = today.getDay() || 7;
            const weekStart = new Date(today);
            weekStart.setDate(today.getDate() - todayDay + 1);
    ...
    getRangeKeys(range) {
        if (!range) return { startKey: null, endKey: null };
        return {
            startKey: this.getDateKey(range.start),
            endKey: this.getDateKey(range.end)
        };
    }

    maybeLoadViewRange() {
        if (this.useNewArchitecture) return false;
        const range = this.getViewDateRange();
        if (!range) return false;
        const { startKey, endKey } = this.getRangeKeys(range);
        if (this.loadedRangeKeys) {
            const loadedStart = this.loadedRangeKeys.startKey;
            const loadedEnd = this.loadedRangeKeys.endKey;
            if (loadedStart && loadedEnd && startKey >= loadedStart && endKey <= loadedEnd) {
                return false;
            }
        }
        if (this.viewDataLoading) return true;
        this.viewDataLoading = true;
        this.loadViewRangeFromCache(startKey, endKey)
            .catch(() => false)
            .finally(() => {
                this.viewDataLoading = false;
                this.render();
    ...
    announceHistoryDataUpdated() {
        try {
            ...
            console.warn('[BrowsingHistoryCalendar] 派发更新事件失败:', error);
        }
    }

    addVisitRecordFromHistory(item, visitTime, options = {}, cutoffTime = 0) {
        if (!item || !item.url || !visitTime) return false;
        if (cutoffTime && visitTime < cutoffTime) return false;

        const visitKey = `${item.url}|${visitTime}`;
        if (this.visitKeySet.has(visitKey)) {
            return false;
        }
    ...
    requestBackgroundCalibration({ reason = 'calendar-refresh', requireIdle = false, clearOnly = false } = {}) {
        return new Promise((resolve) => {
            try {
                const api = (typeof chrome !== 'undefined') ? chrome : (typeof browser !== 'undefined' ? browser : null);
                ...
                const maybePromise = api.runtime.sendMessage({
                    ...
                    payload: { reason, requireIdle, clearOnly }
                }, (response) => {
    ...
    async init() {
        ...
        if (typeof DatabaseManager !== 'undefined') {
            ...
            try {
                ...
                // 从DatabaseManager同步数据到bookmarksByDate（用于渲染）；大数据量时异步分批，避免首屏卡死。
                this.syncFromDatabaseManager({ renderOnComplete: this.isVisible() });
        ...
        if (!this.useNewArchitecture) {
            ...
            if (!restoredFromCache) {
                const result = await this.requestBackgroundCalibration({
                    reason: 'calendar-initial-cache-miss',
                    requireIdle: false
                });
                if (result?.success) {
                    restoredFromCache = await this.restoreBrowsingHistoryCache();
        ...
        if (!savedViewState) {
            // [Modified] Disable auto-jump to history to enforce default "This Week" view
    ...
    isVisible() {
        const container = document.getElementById('additionsView');
     ...
     */
    async syncFromDatabaseManager(options = {}) {
        if (!this.dbManager) return false;
        const syncSeq = ++this.databaseSyncSeq;
        await yieldBrowsingClickDerivation();

        // 从存储库3（书签历史）获取所有记录
        const bookmarkHistoryDB = this.dbManager.getBookmarkHistoryDB();
        const bookmarkDB = this.dbManager.getBookmarksDB?.();
        const refs = {
            urls: bookmarkDB?.getAllUrls?.() || new Set(),
            titles: bookmarkDB?.getAllTitles?.() || new Set(),
            titleDomainMap: new Map()
        };
        ...
        for (const [dateKey, records] of allRecords.entries()) {
            if (syncSeq !== this.databaseSyncSeq) return false;
            const formattedRecords = [];
            for (const record of (Array.isArray(records) ? records : [])) {
                processed += 1;
                if (this.isBookmarkClickRecord(record, refs)) {
                    formattedRecords.push(this.normalizeBookmarkClickRecord(record, { visitKeySet: nextVisitKeySet }));
                }
                if (shouldYieldBrowsingClickDerivation(processed)) {
                    await yieldBrowsingClickDerivation();
                }
            ...
            if (formattedRecords.length) {
                nextBookmarksByDate.set(dateKey, formattedRecords);
            }
        }
        ...
        if (options.announceOnComplete) {
            this.announceHistoryDataUpdated();
        }
        if (options.renderOnComplete && this.isVisible()) {
            this.render();
        }
        return true;
    }
    ...
    restoreSelectMode() {
        ...
        // const savedViewState = localStorage.getItem('browsingHistoryCalendar_viewState'); // [Modified] Disable view persistence
        ...
        */
        ...
        // 恢复视图状态（级别、年月周日）
        // [Modified] Disable view persistence to enforce default Week View (User Request)
        /*
        if (savedViewState) {
            try {
    ...
    saveSelectMode() {
        // [Modified] Disable persistence
        /*
        localStorage.setItem('browsingHistoryCalendar_selectMode', this.selectMode ? 'true' : 'false');

        if (this.selectMode) {
            localStorage.setItem('browsingHistoryCalendar_selectedDates', JSON.stringify(Array.from(this.selectedDates)));
        }
        ...
        localStorage.setItem('browsingHistoryCalendar_viewState', JSON.stringify(viewState));
        */
    }

    // 清除勾选模式的临时记忆
    clearSelectModeMemory() {
        localStorage.removeItem('browsingHistoryCalendar_selectMode');
        localStorage.removeItem('browsingHistoryCalendar_selectedDates');
        localStorage.removeItem('browsingHistoryCalendar_viewState');
    }

    // 更新勾选模式按钮的视觉状态
    updateSelectModeButton() {
        const btn = document.getElementById('browsingCalendarSelectModeBtn');
        if (!btn) return;
    ...
    async loadBookmarkData(options = {}) {
        const { incremental = false } = options;
        try {
            const result = await this.requestBackgroundCalibration({
                reason: incremental ? 'calendar-cache-incremental-refresh' : 'calendar-cache-full-refresh',
                requireIdle: false
            });
            if (!result?.success) {
                console.warn('[BrowsingHistoryCalendar] 后台浏览历史校准失败:', result?.error || 'unknown');
                return;
            }
            if (this.loadedRangeKeys && this.loadedRangeKeys.startKey && this.loadedRangeKeys.endKey) {
                await this.loadViewRangeFromCache(this.loadedRangeKeys.startKey, this.loadedRangeKeys.endKey);
            } else {
                await this.restoreBrowsingHistoryCache();
            }
            this.announceHistoryDataUpdated();
            if (this.isVisible()) {
                this.render();
    ...
    collectBookmarkUrlsAndTitles(node, urlSet, titleSet, parentPath = [], titleDomainMap = null) {
        if (node.url) {
            urlSet.add(node.url);
            // 同时收集标题（去除空白后存储）
            if (node.title && node.title.trim()) {
                ...
                if (titleDomainMap) {
                    const domain = getUrlDomain(node.url);
                    if (domain) {
        ...
        if (node.children) {
            ...
            node.children.forEach(child => this.collectBookmarkUrlsAndTitles(child, urlSet, titleSet, currentPath, titleDomainMap));
        }
    }
    ...
    collectBookmarkUrls(node, urlSet) {
        ...
        if (node.children) {
            node.children.forEach(child => this.collectBookmarkUrls(child, urlSet));
        }
    }

    // 保留原有的parseBookmarks方法签名，但不再使用
    parseBookmarks(node, parentPath = []) {
        // 此方法在BrowsingHistoryCalendar中不再使用
        // 保留是为了兼容性
    }
    getDateKey(date) {
    ...
    setupBreadcrumb() {
        document.getElementById('browsingBreadcrumbYear')?.addEventListener('click', () => this.navigateToYear());
        document.getElementById('browsingBreadcrumbMonth')?.addEventListener('click', () => this.navigateToMonth());
        document.getElementById('browsingBreadcrumbWeek')?.addEventListener('click', () => this.navigateToWeek());

        // 勾选模式按钮
        document.getElementById('browsingCalendarSelectModeBtn')?.addEventListener('click', () => this.toggleSelectMode());

        // 定位至今天按钮
        document.getElementById('browsingCalendarLocateTodayBtn')?.addEventListener('click', () => this.locateToToday());

        // 导出按钮
        document.getElementById('browsingCalendarExportBtn')?.addEventListener('click', () => {
            this.currentExportScope = null;
            this.currentExportScopeTitle = '';
    ...
    populateDropdownMenu(targetId, menu) {
        ...
        // 辅助函数：统计指定年份的记录数量
        const countRecordsInYear = (year) => {
            let count = 0;
            for (const [dateKey, records] of this.bookmarksByDate) {
        ...
        // 辅助函数：统计指定月份的记录数量
        const countRecordsInMonth = (year, month) => {
            let count = 0;
        ...
        // 辅助函数：统计指定周的记录数量
        const countRecordsInWeek = (weekStart) => {
            let count = 0;
            for (let i = 0; i < 7; i++) {
        ...
        if (targetId === 'browsingBreadcrumbYear') {
            ...
            for (let year = endYear; year >= startYear; year--) {
                const count = countRecordsInYear(year);
            ...
            for (let month = 0; month < 12; month++) {
                const count = countRecordsInMonth(this.currentYear, month);
            ...
            while (weekStart <= lastDayOfMonth) {
                ...
                const count = countRecordsInWeek(weekStartCopy);
    ...
    selectYear(year) {
        ...
        for (const [dateKey, bookmarks] of this.bookmarksByDate) {
            if (dateKey.startsWith(`${year}-`)) {
                ...
                if (!firstDate) firstDate = new Date(dateKey);
            }
        }
     ...
     * 高亮（脉冲）当前选中的格子
     */
    // [Added] Select multiple specific dates (Group Search)
    selectDateKeys(dateKeys) {
        ...
        dateKeys.forEach(key => {
            if (this.bookmarksByDate.has(key)) {
                ...
                if (!firstDate) firstDate = new Date(key);
            }
        });
    ...
    pulseSelectedCells() {
        setTimeout(() => {
            const cells = document.querySelectorAll('.calendar-day.selected');
            cells.forEach(cell => {
                cell.classList.add('breathe-animation');
                setTimeout(() => cell.classList.remove('breathe-animation'), 1200);
            });
        }, 100);
    }

    /**
     * [New] Select filtered bookmarks (keyword search mode)
     * Only displays the matching bookmarks, not all bookmarks for the selected dates
     * @param {Object} filteredBookmarks - { dateKey: [bookmark, ...], ... }
     * @param {string} searchQuery - The search query for reference
     */
    selectFilteredBookmarks(filteredBookmarks, searchQuery) {
        if (!filteredBookmarks || typeof filteredBookmarks !== 'object') return;

        this.selectMode = true;
        this.filteredMode = true; // [New] Flag for filtered display
        this.filteredBookmarksData = filteredBookmarks; // [New] Store filtered data
        this.filteredSearchQuery = searchQuery || ''; // [New] Store query
        this.updateSelectModeButton();
        this.selectedDates.clear();

        // Add valid dates to selection
        let firstDate = null;
        const dateKeys = Object.keys(filteredBookmarks).sort();
    ...
    toggleSelectMode() {
        ...
        if (this.selectMode) {
            ...
            // [New] Clear filtered mode state
            this.filteredMode = false;
            this.filteredBookmarksData = null;
            this.filteredSearchQuery = '';
        }
        this.render();
    }
    ...
    updateBreadcrumb() {
        ...
        // 年
        const yearBtn = document.getElementById('browsingBreadcrumbYear');
        document.getElementById('browsingBreadcrumbYearText').textContent = t('calendarYear', this.currentYear);
        ...
        // 月
        const monthBtn = document.getElementById('browsingBreadcrumbMonth');
        const sep1 = document.getElementById('browsingSeparator1');
        if (this.viewLevel === 'month' || this.viewLevel === 'week' || this.viewLevel === 'day') {
            monthBtn.style.display = 'block';
            sep1.style.display = 'inline';
            document.getElementById('browsingBreadcrumbMonthText').textContent = tm(this.currentMonth);
        ...
        // 周
        const weekBtn = document.getElementById('browsingBreadcrumbWeek');
        const sep2 = document.getElementById('browsingSeparator2');
        if (this.viewLevel === 'week' || this.viewLevel === 'day') {
            ...
            document.getElementById('browsingBreadcrumbWeekText').textContent = t('calendarWeek', weekNum);
        ...
        // 日
        const dayBtn = document.getElementById('browsingBreadcrumbDay');
        const sep3 = document.getElementById('browsingSeparator3');
        if (this.viewLevel === 'day') {
            dayBtn.style.display = 'block';
            sep3.style.display = 'inline';
            document.getElementById('browsingBreadcrumbDayText').textContent =
    ...
    render() {
        // [Performance] Guard: Don't render if hidden
        if (this.isVisible && !this.isVisible()) {
            return;
        }
        if (this.maybeLoadViewRange()) {
            return;
        }

        const container = document.getElementById('browsingCalendarView');
        if (!container) return;
    ...
    createMonthBookmarksList() {
        ...
        // [Fixed] 勾选模式下遍历所有选中的日期（跨月），普通模式下只遍历当前月份
        if (this.selectMode && this.selectedDates.size > 0) {
            // 勾选模式：遍历所有选中的日期（可能跨越多个月份）
            const sortedDateKeys = Array.from(this.selectedDates).sort();
            for (const dateKey of sortedDateKeys) {
                // [New] In filtered mode, use filtered bookmarks; otherwise use all bookmarks
                let bookmarks;
                if (this.filteredMode && this.filteredBookmarksData && this.filteredBookmarksData[dateKey]) {
                    bookmarks = this.filteredBookmarksData[dateKey];
                } else {
                    bookmarks = this.bookmarksByDate.get(dateKey) || [];
                }
        ...
        // 当月：仅在“普通浏览模式”下，才插入一个“今天(0)”的占位条目。
        // 说明：搜索/筛选模式（filteredMode）以及“所有选中的书签”(selectMode)下，
        // 若今天没有数据，不应把今天(0)混入筛选结果里。
        const shouldEnsureTodayPlaceholder = isCurrentMonth && !this.selectMode && !this.filteredMode;
        if (shouldEnsureTodayPlaceholder) {
        ...
        // 兜底：如果没有任何日期条目（例如筛选结果为空），直接显示空态，避免后续 sortedWeeks[0] 报错。
        if (bookmarksByWeek.size === 0) {
        ...
        // 默认选中日期 key：普通浏览模式下当前月优先今天，否则第一周的第一天
        const firstWeekData = bookmarksByWeek.get(sortedWeeks[0]);
        let selectedDateKey = this.getDateKey(firstWeekData[0].date);
        if (isCurrentMonth && !this.selectMode && !this.filteredMode) {
            // 普通模式下优先选中今天（如无数据，上面会插入今天(0)占位，保证菜单存在）
    ...
    createBookmarkItem(bookmark, showTreeLines = false, isLast = false) {
        ...
        item.addEventListener('click', () => {
            notifyBrowsingCalibrationInteraction('click', { source: 'browsing-history-calendar', url: bookmark.url });
    ...
    createFlatBookmarkList(bookmarks) {
        ...
        const renderNextBatch = () => {
            const end = Math.min(renderedCount + BOOKMARK_LOAD_BATCH_SIZE, bookmarks.length);
            for (let i = renderedCount; i < end; i++) {
                listHost.appendChild(this.createFlatHistoryBookmarkItem(bookmarks[i]));
            }
            renderedCount = end;
        };
    ...
    setupExportUI() {
        ...
        const doExportBtn = document.getElementById('doBrowsingExportBtn');
    ...
    getSelectedExportHistoryDateKeys() {
        if (!this.selectMode || this.currentExportScope) return [];
    ...
    getExportHistoryContextBounds() {
        ...
        const scope = this.currentExportScope || null;
        if (scope && scope.type === 'hour' && scope.data) {
            const date = new Date(scope.data.date);
            const hour = Number(scope.data.hour);
            if (!Number.isNaN(date.getTime()) && Number.isFinite(hour)) {
                return {
                    startTime: new Date(date.getFullYear(), date.getMonth(), date.getDate(), hour, 0, 0, 0).getTime(),
                    endTime: new Date(date.getFullYear(), date.getMonth(), date.getDate(), hour, 59, 59, 999).getTime()
                };
            }
        }

        if (scope && scope.type === 'day' && scope.data?.date) {
            return dayBounds(scope.data.date);
        }

        if (scope && scope.type === 'week' && scope.data?.weekStart) {
        ...
        if (scope && scope.type === 'all' && scope.data) {
            const data = scope.data;
            if (data.viewLevel === 'day' && data.date) {
                return dayBounds(data.date);
            }
            if (data.viewLevel === 'week' && data.weekStart) {
            ...
            if (data.viewLevel === 'month' && Number.isFinite(Number(data.year)) && Number.isFinite(Number(data.month))) {
                const year = Number(data.year);
                const month = Number(data.month);
                return {
                    startTime: new Date(year, month, 1, 0, 0, 0, 0).getTime(),
        ...
        if (this.selectMode) {
            const dates = this.getSelectedExportHistoryDateKeys();
            if (dates.length > 0) {
                const [startY, startM, startD] = dates[0].split('-').map(Number);
                const [endY, endM, endD] = dates[dates.length - 1].split('-').map(Number);
                return {
                    startTime: new Date(startY, startM - 1, startD, 0, 0, 0, 0).getTime(),
                    endTime: new Date(endY, endM - 1, endD, 23, 59, 59, 999).getTime()
                };
            }
            return null;
        }
        ...
        const range = this.getViewDateRange();
        return range
            ? { startTime: range.start.getTime(), endTime: range.end.getTime() }
            : null;
    }
    ...
    async handleExport() {
        ...
        try {
            ...
            const exportBookmarksByDate = await this.resolveExportBookmarksByDate();

            // 获取数据
            const exportData = await this.getExportData(mode, { bookmarksByDate: exportBookmarksByDate });
            ...
            // 生成文件名（不包含时间戳）
            const filenameBase = this.generateExportFilename(mode, exportBookmarksByDate);
            ...
            if (mode === 'history_context') {
                try {
                    const bounds = this.getExportHistoryContextBounds();
                    if (bounds && typeof readHistoryCacheRange === 'function') {
                        const selectedDateKeys = this.getSelectedExportHistoryDateKeys();
                        const selectedDateSet = selectedDateKeys.length ? new Set(selectedDateKeys) : null;
                        const startKey = this.getDateKey(new Date(bounds.startTime));
                        const endKey = this.getDateKey(new Date(bounds.endTime));
                        const dbData = await readHistoryCacheRange(startKey, endKey).catch(() => null);
                        const filteredHistoryItems = [];
                        if (dbData && Array.isArray(dbData.records)) {
                            for (const [, records] of dbData.records) {
                                for (const rec of (Array.isArray(records) ? records : [])) {
                                    const visitTime = Number(rec.visitTime || rec.dateAdded || 0);
                                    if (!Number.isFinite(visitTime) || visitTime < bounds.startTime || visitTime > bounds.endTime) continue;
                                    if (selectedDateSet) {
                                        const visitDateKey = this.getDateKey(new Date(visitTime));
                                        if (!selectedDateSet.has(visitDateKey)) continue;
                                    }
    ...
    getExportCacheDateRange() {
        if (this.currentExportScope && this.currentExportScope.type) {
            const scope = this.currentExportScope;
            switch (scope.type) {
                case 'week': {
                    if (!scope.data?.weekStart) break;
                    const start = new Date(scope.data.weekStart);
                    start.setHours(0, 0, 0, 0);
                    const end = new Date(start);
                    end.setDate(end.getDate() + 6);
                ...
                case 'day':
                case 'hour': {
                    if (!scope.data?.date) break;
                    const start = new Date(scope.data.date);
                    start.setHours(0, 0, 0, 0);
                    const end = new Date(scope.data.date);
                    end.setHours(23, 59, 59, 999);
                    return { start, end };
                }
                case 'all':
        ...
        if (this.selectMode && this.selectedDates && this.selectedDates.size > 0) {
            const dates = Array.from(this.selectedDates)
                .map(dateKey => {
        ...
        return this.getViewDateRange();
    }

    async buildExportBookmarksByDateFromCache() {
        if (typeof readHistoryCacheRange !== 'function') return null;

        const range = this.getExportCacheDateRange();
        const keys = range && range.start && range.end
            ? this.getRangeKeys(range)
            : { startKey: null, endKey: null };
        const cached = await readHistoryCacheRange(keys.startKey, keys.endKey);
        if (!cached || !Array.isArray(cached.records)) return null;

        const refs = await this.buildBookmarkClickReferenceSets();
        const exportBookmarksByDate = new Map();
        const exportVisitKeySet = new Set();
        let processed = 0;

        for (const [dateKey, records] of cached.records) {
            if (!dateKey || !this.checkDateInScope(dateKey)) continue;
            const normalized = [];
            for (const record of (Array.isArray(records) ? records : [])) {
                processed += 1;
                if (this.isBookmarkClickRecord(record, refs)) {
                    const normalizedRecord = this.normalizeBookmarkClickRecord(record, { visitKeySet: exportVisitKeySet });
                    if (this.checkBookmarkInScope(normalizedRecord)) {
            ...
            if (normalized.length) {
                exportBookmarksByDate.set(dateKey, normalized);
            }
        }

        return exportBookmarksByDate;
    }

    async resolveExportBookmarksByDate() {
        try {
            const fromCache = await this.buildExportBookmarksByDateFromCache();
            if (fromCache instanceof Map) {
                return fromCache;
            }
        } catch (error) {
            console.warn('[BrowsingHistoryCalendar] failed to build export data from history cache:', error);
        }
        return this.bookmarksByDate;
    }

    // 辅助：获取导出范围名称（用于文件夹命名）
    getExportScopeName(bookmarksByDate = this.bookmarksByDate) {
        // 优先使用记录的导出范围标题
    ...
    // 生成导出文件名（根据模式和范围）
    generateExportFilename(mode, bookmarksByDate = this.bookmarksByDate) {
        // 根据模式确定前缀
        let prefix = '';
        let prefixEn = '';
        switch (mode) {
        ...
        if (this.currentExportScopeTitle) {
            ...
            scopeSuffix = this.getExportScopeSuffix(bookmarksByDate);
        }
    ...
    // 获取导出范围后缀（用于文件名）
    getExportScopeSuffix(bookmarksByDate = this.bookmarksByDate) {
        if (this.selectMode) {
            // 勾选模式：过滤掉没有数据的日期
            const dates = Array.from(this.selectedDates)
                .filter(dateKey => {
    ...
    async getExportData(mode, options = {}) {
        ...
        const exportBookmarksByDate = options.bookmarksByDate instanceof Map
            ? options.bookmarksByDate
            : await this.resolveExportBookmarksByDate();
        ...
        const mainFolderName = this.getExportScopeName(exportBookmarksByDate);
        const mainFolder = {
        ...
        // 2. 收集所有符合范围的书签（按日期排序）
        const sortedDateKeys = [...exportBookmarksByDate.keys()].sort();
        ...
        if (mode === 'records') {
            ...
            for (const dateKey of sortedDateKeys) {
                ...
                const bookmarks = exportBookmarksByDate.get(dateKey);
...
`;
    ...
    exitLocateMode() {
        ...
        if (container) {
            ...
            // 移除所有高亮
            const highlighted = container.querySelectorAll('.calendar-day.search-highlight');
    ...
    renderExitButton() {
        // Append to body to ensure fixed positioning works correctly relative to viewport
        if (document.getElementById('exitLocateModeBtn')) return;

        const btn = document.createElement('button');
        btn.id = 'exitLocateModeBtn';
        btn.className = 'exit-locate-mode-btn';
        btn.innerHTML = '<i class="fas fa-times"></i> ' + (typeof currentLang !== 'undefined' && currentLang === 'zh_CN' ? '退出定位' : 'Exit Positioning');
        btn.onclick = (e) => {
            e.stopPropagation();
            this.exitLocateMode();
        }
...
// 初始化
window.initBrowsingHistoryCalendar = function () {
    if (window.browsingHistoryCalendarInstance) {
        window.browsingHistoryCalendarInstance.render();
        return;
    }

    window.browsingHistoryCalendarInstance = new BrowsingHistoryCalendar();
};

// 语言切换时更新日历翻译
window.updateBrowsingHistoryCalendarLanguage = function () {
    if (window.browsingHistoryCalendarInstance) {
        window.browsingHistoryCalendarInstance.render();
    }
};

// 不自动初始化，只在点击标签时初始化（在 history.js 的 initAdditionsSubTabs 中调用）
