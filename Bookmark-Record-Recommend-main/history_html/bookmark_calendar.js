...
class BookmarkCalendar {
    ...
    restoreSelectMode() {
        ...
        // const savedViewState = localStorage.getItem('bookmarkCalendar_viewState'); // [Modified] Disable view persistence to enforce default Week View
        const savedSortAsc = localStorage.getItem('bookmarkCalendar_sortAsc');

        /* [Modified] Disable selection persistence (User Request: Refresh exits locate mode)
        ...
        */
        ...
        /* [Modified] Disable selection persistence
    ...
    getTreeViewStorageKey() {
        ...
        return `bookmarkCalendar_treeViewMode_${suffix}`;
    }
    ...
    saveSelectMode() {
        // [Modified] Disable persistence (User Request: Refresh exits locate mode)
        /*
    ...
    preloadFavicons() {
        // 收集有限数量的唯一域名，避免在后台把整棵书签树的 favicon 都读入内存。
    ...
    selectDay(dateKey) {
        ...
        this.viewLevel = 'month'; // Keep month view for context, or switch to day? User said "Green Box", usually implies seeing it in grid. Month view is safer.
    ...
    syncMonthViewHeights(rootContainer = null) {
        const root = rootContainer || document.getElementById('bookmarkCalendarView');
        if (!root) return;
    ...
    createMonthCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            dayCell.innerHTML = `
                ...
                ${isTodayCell ? `<div style="position: absolute; bottom: ${todayHintInset}; right: ${todayHintInset}; font-size: ${todayHintSize}; color: #2196F3; font-weight: 600;">${(typeof currentLang !== 'undefined' && currentLang === 'en') ? 'Today' : '今天'}</div>` : ''}
            `;
    ...
    createCollapsibleBookmarkList(bookmarks, containerId) {
        ...
        const bookmarksWithPath = sortedBookmarks.map((bookmark) => ({
            ...bookmark,
            folderPath: Array.isArray(bookmark.folderPath) ? bookmark.folderPath : []
        }));
    ...
    createFlatBookmarkList(bookmarks) {
        return this.renderBookmarkList(bookmarks, false);
    }
    ...
    renderWeekView(container) {
        ...
        if (allBookmarks.length === 0) {
            ...
            emptyState.innerHTML = `<i class="fas fa-calendar-week"></i><p>${currentLang === 'en' ? 'No bookmarks this week' : '本周暂无书签'}</p>`;
    ...
    renderYearView(container) {
        ...
        if (!hasMonthData) {
            const emptyState = document.createElement('div');
            emptyState.className = 'calendar-empty-state';
            emptyState.innerHTML = `<i class="fas fa-calendar-alt"></i><p>${currentLang === 'en' ? 'No bookmarks this year' : '本年暂无书签'}</p>`;
            wrapper.appendChild(emptyState);
            container.appendChild(wrapper);
            return;
        }
    ...
    createInlineExportButton(scopeData) {
        ...
        tooltip.textContent = currentLang === 'zh_CN' ? '导出书签添加记录' : 'Export Bookmark Addition Records';
    ...
    generateExportFilename(mode) {
        ...
        switch (mode) {
            case 'records':
                prefix = '书签添加记录';
                prefixEn = 'bookmark_addition_records';
                break;
            case 'context':
                prefix = '书签添加记录_上下文';
                prefixEn = 'bookmark_addition_context';
                break;
            case 'collection':
                prefix = '书签添加记录_集合';
                prefixEn = 'bookmark_addition_collection';
                break;
            default:
                prefix = '书签添加记录';
                prefixEn = 'bookmark_addition_records';
...
`;
    ...
    downloadFile(content, filename, type, timestamp = Date.now()) {
        ...
        if (chrome.downloads) {
            ...
            const exportPath = `${getCalendarExportRootFolder()}/${getCalendarManualExportFolder()}/${getCalendarExportFolder()}/${exportDateKey}`;
