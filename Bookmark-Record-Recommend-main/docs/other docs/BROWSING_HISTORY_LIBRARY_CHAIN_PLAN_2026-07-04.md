...
- The local IndexedDB cache is the cross-90-day source of truth for bookmark-related browsing records.
- Calibration refreshes the hot window and must not erase cold records unless the user explicitly purges them or a privacy delete policy requires it.
...
4. Legacy storage migration overwrites same-day records instead of merging by visit key.
...
- Migration merges same-day records by `url + visitTime`.
...
1. Patch background metadata, migration, retry, truncation, and purge correctness.
...
- Done: legacy cache migration merges same-day records by visit key instead of overwriting date buckets.
